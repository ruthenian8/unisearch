...
@pytest.parametrize("arg", ["?query=лопата", "?query=лагерь"])
...
@pytest.parametrize("arg", ["" "?url=" "?url=sudo rm -rf ../*", "?url=foo bar baz"])
def test_parse_incorrect(test_client, arg):
    response = test_client.get("/parse" + arg)
    assert response.status_code == 400


@pytest.parametrize("arg", ["?url=https://shalamov.ru/library/6/"])
